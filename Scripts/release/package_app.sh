...
<dict>
    ...
    <key>NSCameraUsageDescription</key><string>THOR uses the camera to preview and bridge host-connected robot sensors into the THOR simulator.</string>
...
if [[ -d "$ROOT/Agent" ]]; then
  cp -R "$ROOT/Agent" "$APP/Contents/Resources/Agent"
fi

if [[ -d "$ROOT/Scripts/jetson-thor" ]]; then
  mkdir -p "$APP/Contents/Resources/jetson-thor"
  cp -R "$ROOT/Scripts/jetson-thor/." "$APP/Contents/Resources/jetson-thor/"
  chmod +x "$APP/Contents/Resources/jetson-thor/"*.sh 2>/dev/null || true
fi

if [[ -d "$ROOT/docs/setup" ]]; then
  mkdir -p "$APP/Contents/Resources/setup"
  cp -R "$ROOT/docs/setup/." "$APP/Contents/Resources/setup/"
fi
